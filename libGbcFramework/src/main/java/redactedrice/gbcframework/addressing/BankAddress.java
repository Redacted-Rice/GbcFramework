...
public class BankAddress {
    ...
    public static final BankAddress UNASSIGNED = new BankAddress(UNASSIGNED_BANK,
            UNASSIGNED_ADDRESS);
    public static final BankAddress ZERO = new BankAddress((byte) 0, (short) 0);

    public enum BankAddressLimitType {
        IN_VALID_RANGES, WITHIN_BANK, WITHIN_BANK_OR_START_OF_NEXT
    }

    public enum BankAddressToUseType {
        BANK_AND_BANK_IN_ADDRESS, BANK_ONLY, ADDRESS_IN_BANK_ONLY
    }
    ...
    public BankAddress(int globalAddress) {
        if (globalAddress < 0
                || globalAddress >= RomConstants.NUMBER_OF_BANKS * RomConstants.BANK_SIZE) {
            throw new IllegalArgumentException("BankAddress - invalid globalAddress given ("
                    + globalAddress + ") - the globalAddress must be between 0 and "
                    + RomConstants.NUMBER_OF_BANKS * RomConstants.BANK_SIZE);
        }
    ...
    public BankAddress newAbsoluteDifferenceBetween(BankAddress toGetRelativeOf) {
        return newAbsoluteDifferenceBetween(toGetRelativeOf,
                BankAddressToUseType.BANK_AND_BANK_IN_ADDRESS);
    }

    public BankAddress newAbsoluteDifferenceBetween(BankAddress toGetRelativeOf,
            BankAddressToUseType whatToDiff) {
        switch (whatToDiff) {
        case BANK_ONLY:
            return new BankAddress((byte) Math.abs(bank - toGetRelativeOf.bank), (short) 0);
        case ADDRESS_IN_BANK_ONLY:
            return new BankAddress((byte) 0,
                    (short) Math.abs(addressInBank - toGetRelativeOf.addressInBank));
        default: // Default to all
        case BANK_AND_BANK_IN_ADDRESS:
            return new BankAddress(Math.abs(
                    getDifference(toGetRelativeOf, BankAddressToUseType.BANK_AND_BANK_IN_ADDRESS)));
        }
    }
    ...
    public boolean add(BankAddress toAdd) {
        return add(toAdd, BankAddressToUseType.BANK_AND_BANK_IN_ADDRESS,
                BankAddressLimitType.IN_VALID_RANGES);
    }
    ...
    public boolean add(BankAddress toAdd, BankAddressToUseType whatToAdd,
        ...
        if (valid) {
            ...
            case IN_VALID_RANGES:
                ...
                break;
            case WITHIN_BANK_OR_START_OF_NEXT:
                valid = bankSum == bank || (bankSum - 1 == bank && addressSum == 0);
                break;
            case WITHIN_BANK:
                valid = bankSum == bank;
                break;
    ...
    public void setAddressInBank(short addressInBank) {
        if (!isAddressInBankInRange(bank, addressInBank, BankAddressLimitType.WITHIN_BANK)) {
            throw new IllegalArgumentException("BankAddress - invalid addressInBank given ("
                    + addressInBank + ") - the bank must be between 0 and " + RomConstants.BANK_SIZE
    ...
    private static boolean isAddressInBankInRange(int bankToCheck, int addrToCheck,
            BankAddressLimitType type) {
        switch (type) {
        default:
        case IN_VALID_RANGES:
            while (!isAddressInBankInRange(bankToCheck, addrToCheck,
                    BankAddressLimitType.WITHIN_BANK)) {
                bankToCheck++;
        ...
        case WITHIN_BANK_OR_START_OF_NEXT:
            return (addrToCheck == UNASSIGNED_ADDRESS
                    || addrToCheck >= 0 && (addrToCheck < RomConstants.BANK_SIZE
                            || (type == BankAddressLimitType.WITHIN_BANK_OR_START_OF_NEXT
                                    && addrToCheck == RomConstants.BANK_SIZE)));
        }
    }
    ...
    public boolean offsetFits(int offset) {
        return isAddressInBankInRange(bank, this.addressInBank + offset,
                BankAddressLimitType.IN_VALID_RANGES);
    }
    ...
    public int getDifference(BankAddress other, BankAddressToUseType whatToDiff) {
        ...
        default: // Default to all
        case BANK_AND_BANK_IN_ADDRESS:
            return (other.bank - bank) * RomConstants.BANK_SIZE + other.addressInBank
                    - addressInBank;
        case BANK_ONLY:
    ...
    public boolean equals(Object o, BankAddressToUseType whatToCheck) {
        ...
        case BANK_AND_BANK_IN_ADDRESS:
            return Byte.compare(bank, ba.bank) == 0
                    && Short.compare(addressInBank, ba.addressInBank) == 0;
        case BANK_ONLY:
            return Byte.compare(bank, ba.bank) == 0;
        case ADDRESS_IN_BANK_ONLY:
    ...
    public String toString() {
        return String.format("0x%x:%4x(%d)", bank, addressInBank + 0x4000,
                RomUtils.convertToGlobalAddress(bank, addressInBank));
    }
}
