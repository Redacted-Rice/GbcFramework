...
public class AddressRange {
    ...
    public AddressRange(int start, int stopExclusive) {
        if (start > stopExclusive) {
            throw new IllegalArgumentException("AddressRange: The exclusive stop passed ("
                    + stopExclusive + ") was before the start (" + start + ")");
        }
        this.start = start;
    ...
    public AddressRange(BankAddress start, int size) {
        if (size < 0) {
            throw new IllegalArgumentException(
                    "AddressRange: The size passed (" + size + ") is not a positive number");
        }
    ...
    public boolean overlaps(AddressRange toCheck) {
        ...
        return contains(toCheck.start) || contains(toCheck.stopExclusive - 1) || // -1 on end since
                                                                                 // its not
                                                                                 // inclusive
    ...
    public static void sortAndCombine(List<AddressRange> ranges) {
        ...
        if (ranges.size() > 1) {
            ...
            Collections.sort(sorted, (ar1, ar2) -> ar1.start - ar2.start);
